...
from .spim_inverted_index_memory_binary import (
    SinglePassInMemoryInvertedIndexMemoryBinary,
    read_token,
    write_token,
    write_doc_ids
)
...
@pytest.fixture
def spim_index(idx_dir) -> SinglePassInMemoryInvertedIndexMemoryBinary:
    yield SinglePassInMemoryInvertedIndexMemoryBinary(idx_dir)
...
def test_spim_restore(spim_index):
    with open(os.path.join(spim_index.idx_dir, 'inverted_index'), 'wb') as f:
        f.write(B_INT16_1 + b'a' + B_INT32_1 + B_INT32_1 +
                ...
                B_INT16_1 + b'c' + B_INT32_2 + B_INT32_0 + B_INT32_1)
    spim_index.restore()
    assert spim_index.data == {'a': [B_INT32_1], 'b': [B_INT32_0], 'c': [B_INT32_0, B_INT32_1]}
