...
class SinglePassInMemoryInvertedIndexMemory(InvertedIndex):
    ...
    def prepare_state(self, tokens: list[str]) -> list[(int, list[int])]:
        state = []
        for t in tokens:
            ids = self.data.get(t, [])
            if not ids:
    ...
    def binary_search(self, a: list[int], doc_id: int, left: int, right: int) -> int:
    ...
    def search_and(self, tokens: list[str]) -> list[int]:
        ...
        # find common doc ids
        for i, (n_b, ids_b) in enumerate(state[1:]):
            result = []
            self.double_binary_search(doc_ids, 0, len(doc_ids), ids_b, 0, n_b, result)
            doc_ids = result
        return doc_ids

    def count_and(self, tokens: list[str]) -> int:
        if len(tokens) == 1:
            return len(self.get(tokens[0]))
